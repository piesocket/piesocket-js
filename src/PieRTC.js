...
export default class PieRTC {
  /**
   * Creates a video room instance over a v4 (multiplexed) channel
   * @param {*} channel
   * @param {*} identity
   *
   * @return {void} Returns an instance of PieRTC
   */
  constructor(channel, identity) {
    this.channel = channel;
    this.logger = new Logger(identity);
    this.identity = {...defaultRTCOptions, ...identity};
  ...
  shareVideo(signal, isCaller = true) {
    ...
    rtcConnection.onicecandidate = (event) => {
      if (event.candidate != null) {
        this.channel.publish('rtc::candidate', {
  ...
  async onLocalScreen(screenStream) {
    ...
    screenStream.getVideoTracks()[0].addEventListener('ended', () => {
      this.channel.publish('rtc::stopped_screen', {
        from: this.channel.uuid,
  ...
  async sendVideoOffer(signal, rtcConnection, isCaller) {
    ...
    // Send a call offer
    this.channel.publish('rtc::offer', {
      from: this.channel.uuid,
      to: signal.from,
  ...
  createAnswer(signal) {
    return new Promise(async (resolve, reject) => {
      ...
      if (signal.sdp.type == 'offer') {
        ...
        this.channel.publish('rtc::answer', {
          from: this.channel.uuid,
          to: signal.from,
  ...
  requestPeerVideo() {
    let eventName = 'rtc::broadcaster';

    if (!this.identity.shouldBroadcast) {
      eventName = 'rtc::watcher';
    }
  ...
  requestOfferFromPeer() {
    this.channel.publish('rtc::request', {
      from: this.channel.uuid,
      isBroadcasting: this.identity.shouldBroadcast,
    });
  }
  ...
  errorHandler(e) {
    this.logger.error('PieRTC error', e);
  }
}
